...
尚未执行在线鉴权或真实飞书写入；L1/L2/L3 必须分别按手册取得对应授权。

### 门禁

280 项测试通过、分支覆盖率 90.56%、场景回放 9/9、离线评测 34 样本全部通过、Ruff 与
`git diff --check` 全净。用当前本地 `.env` 执行零网络预检时，适配器、凭证存在性、WebSocket
和写入闸门均通过，但 `ORGPILOT_DEMO_BOOTSTRAP=true` 被安全门拒绝；进入 L1 前需由操作者
明确决定是否把本地演示模式关闭。
